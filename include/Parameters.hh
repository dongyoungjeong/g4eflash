...
static const G4double SP_D = 5.*cm;
static const G4double PHANTOM_D = 50. * cm;

// Phantom geometry and scoring parameters
static const G4int PHANTOM_NTUPLE_OFFSET = 1; // ntuple 0 is reserved for head SD
static const G4double PLANE_THICKNESS = mm; // should be consistent with the actual geometry
static const G4double PHANTOM_R = 10. * cm;
static const G4double PHANTOM_Z = 10. * cm; // should be consistent with the actual geometry
static const G4int N_PLANES = lround(PHANTOM_Z/PLANE_THICKNESS); // number of planes in the phantom, should be consistent with the actual geometry
